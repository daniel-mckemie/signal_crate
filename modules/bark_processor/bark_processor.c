...
static inline float center_window_bark(const BarkProcessor* s, int b, float center01, float width01) {
...
static inline float soft_sat(float x, float drive) {
    if (drive <= 0.0f) return x;                 /* true bypass */
...
static inline float biquad_tick_state(
    const BarkProcessor* s, int b, int st, float x, float* z1, float* z2
) {
...
/* Verbos-style follower: instant attack, fixed release (~80ms). No user atk/rel params. */
...
static void clamp_params(BarkProcessor* s) {
    ...
    if (s->odd_to_even) s->odd_to_even = 1;
    if (s->even_to_odd) s->even_to_odd = 1;
...
static void bark_processor_process(Module* m, float* in, unsigned long frames) {
    ...
    if (!in_odd)  in_odd  = in_even;
    if (!in_even) in_even = in_odd;
    ...
    for (unsigned int i=0;i<frames;i++) {
        ...
        for (int j=0; j<m->num_control_inputs; j++) {
            ...
            else if (strcmp(param,"out_odd")==0 || strcmp(param,"outgain_odd")==0) og_odd += control;
            else if (strcmp(param,"out_even")==0 || strcmp(param,"outgain_even")==0) og_even += control;

            else if (strcmp(param,"odd2even")==0) o2e = (control > 0.0f);
            else if (strcmp(param,"even2odd")==0) e2o = (control > 0.0f);
        ...
        for (int b=0;b<BARK_PROC_BANDS;b++) {
            ...
            if (pair < 0 || pair >= BARK_PROC_BANDS) pair = b;
...
static void bark_processor_draw_ui(Module* m, int y, int x) {
    ...
    LABEL(2,"ogo:"); ORANGE(); printw("%.2f", ogo);    CLR();
    LABEL(2,"oge:"); ORANGE(); printw("%.2f", oge);    CLR();
    LABEL(2,"o2e:"); ORANGE(); printw("%d", o2e);      CLR();
    LABEL(2,"e2o:"); ORANGE(); printw("%d", e2o);      CLR();
    LABEL(2,"b:"); ORANGE(); printw("%02d", sb);       CLR();
    LABEL(2,"g:"); ORANGE(); printw("%.2f", sg);       CLR();

    YELLOW();
    mvprintw(y+1, x, "-/= ctr _/+ wid  [/] tilt  {/} drv  i/I og_odd  k/K og_even  o o2e  p e2o  b/B band  ;/' gain");
    mvprintw(y+2, x, ":1[ctr] :2[wid] :3[tilt] :4[drv] :i[og_odd] :k[og_even] :6[band] :7[gain] :o[o2e] :p[e2o]");
    BLACK();
}
...
static void bark_processor_handle_input(Module* m, int key) {
    ...
    if (!s->entering_command) {
        switch (key) {
            ...
            case 'I': s->out_gain_odd  += 0.01f; handled=1; break;
            case 'i': s->out_gain_odd  -= 0.01f; handled=1; break;
            case 'K': s->out_gain_even += 0.01f; handled=1; break;
            case 'k': s->out_gain_even -= 0.01f; handled=1; break;

            case 'o': s->odd_to_even ^= 1; handled=1; break;
            case 'p': s->even_to_odd ^= 1; handled=1; break;
        ...
        if (key == '\n') {
            ...
            if (sscanf(s->command_buffer, "%c %f", &type, &val) >= 1) {
                ...
                else if (type=='i') s->out_gain_odd = val;
                else if (type=='k') s->out_gain_even = val;
                else if (type=='6') s->sel_band = (int)val;
                else if (type=='7') s->band_gain[s->sel_band] = val;
                else if (type=='o') s->odd_to_even = (val > 0.5f);
                else if (type=='p') s->even_to_odd = (val > 0.5f);
            }
            handled=1;
        } else if (key == 27) {
...
static void bark_processor_set_osc_param(Module* m, const char* param, float value) {
    ...
    else if (strcmp(param,"in_odd")==0 || strcmp(param,"ingain_odd")==0) s->out_gain_odd = value;
    else if (strcmp(param,"in_even")==0 || strcmp(param,"ingain_even")==0) s->out_gain_even = value;

    else if (strcmp(param,"odd2even")==0) s->odd_to_even = (value > 0.5f);
    else if (strcmp(param,"even2odd")==0) s->even_to_odd = (value > 0.5f);
...
Module* create_module(const char* args, float sample_rate) {
    ...
    if (args && strstr(args,"in_odd="))   sscanf(strstr(args,"in_odd="),  "in_odd=%f",  &s->out_gain_odd);
    if (args && strstr(args,"in_even="))  sscanf(strstr(args,"in_even="), "in_even=%f", &s->out_gain_even);

    if (args && strstr(args,"odd2even=")) sscanf(strstr(args,"odd2even="),"odd2even=%d",&s->odd_to_even);
    if (args && strstr(args,"even2odd=")) sscanf(strstr(args,"even2odd="),"even2odd=%d",&s->even_to_odd);
