...
float get_cpu_percent() {
    ...
    double delta_user =
        (usage_now.ru_utime.tv_sec - last_usage.ru_utime.tv_sec) +
        (usage_now.ru_utime.tv_usec - last_usage.ru_utime.tv_usec) / 1e6;
    double delta_sys =
        (usage_now.ru_stime.tv_sec - last_usage.ru_stime.tv_sec) +
...
void ui_loop() {
    if (!ui_enabled) {
        ...
        while (1)
            usleep(100000); // keep audio thread alive
        return;
    }
    ...
    while (running) {
        ...
        if (mchan > 0 && mcc >= 0)
            mvprintw(0, COLS - 54,
                     "[CPU] %.1f%%  [OSC:%s]  [MIDI ch:%d cc:%d] [%02d:%02d]",
                     cpu, osc_port, mchan, mcc, mm, ss);
        else
            mvprintw(0, COLS - 54, "[CPU] %.1f%%  [OSC:%s] [%02d:%02d]", cpu,
                     osc_port, mm, ss);
        ...
        int modules_per_col =
            (rows - 4) / (base_module_height + module_spacing);
        if (modules_per_col < 1)
            modules_per_col = 1;
        ...
        if (in_command_mode) {
            ...
            attrset(A_NORMAL);
            mvprintw(LINES - 2, 2,
                     "[TAB] switch module | [t] show/hide cmds | [:q] quit | "
                     "[:] cmd mode | [ESCx2] exit cmd mode");
        }
        ...
        if (in_command_mode) {
            ...
            if (ch == '\t') {
                focused_module_index =
                    (focused_module_index + 1) % module_count;
