    ...
    for (int i = 0; i < frames; i++) {
        ...
        if (s->phase >= 1.0f) {
            ...
            case RAND_WHITE:
                break;
            case RAND_PINK:
                shaped = pink_filter_process(&s->pink, base);
                break;
            case RAND_BROWN:
                shaped = brown_noise_process(&s->brown, base);
                break;
            }
...
static void c_random_draw_ui(Module *m, int y, int x) {
    ...
    YELLOW();
    mvprintw(y + 1, x,
             "Real-time keys: -/= (rate), [/] (min), {/} (max), n (type), d/D "
             "(depth)");
    mvprintw(y + 2, x,
             "Cmd: :1 [rate], :2 [rmin], :3 [rmax], :4 [type] :d [depth]");
    BLACK();
}
...
Module *create_module(const char *args, float sample_rate) {
    ...
    if (args && strstr(args, "type=")) {
        ...
        if (strcmp(buf, "white") == 0)
            type = RAND_WHITE;
        else if (strcmp(buf, "pink") == 0)
            type = RAND_PINK;
        else if (strcmp(buf, "brown") == 0)
            type = RAND_BROWN;
    }
