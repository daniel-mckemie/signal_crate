...
static void player_process(Module *m, float *in, unsigned long frames) {
    ...
    for (unsigned long i = 0; i < frames; i++) {
        ...
        if (i1 < 0)
            i1 = 0;
        if (i1 > (sf_count_t)(max_frames - 2))
            i1 = (sf_count_t)(max_frames - 2);
        sf_count_t i2 = i1 + 1;
...
static void player_draw_ui(Module *m, int y, int x) {
    ...
    LABEL(2, "");
    ORANGE();
    printw(" %.1fs/%.1fs (%s)|", pos_sec, dur_sec, is_playing ? "p" : "s");
    CLR();
    ...
    LABEL(2, "loop:");
    ORANGE();
    printw(" %s", loop ? "on" : "off");
    CLR();

    YELLOW();
    mvprintw(
        y + 1, x,
        "keys: -/= scrub | _/+ (speed) | [/] (amp) | p=play, s=stop, l=loop");
        ...
        if (key == '\n') {
            ...
            if (sscanf(state->command_buffer, "%c%f", &type, &val) == 2) {
                if (type == '1') {
                    ...
                    if (new_pos < 0)
                        new_pos = 0;
                    if (new_pos > state->num_frames - 1)
                        new_pos = state->num_frames - 1;
...
Module *create_module(const char *args, float sample_rate) {
    ...
    if (args && strstr(args, "file=")) {
        ...
        size_t i = 0;
        while (*file_arg && *file_arg != ',' && *file_arg != ' ' &&
               i < sizeof(filepath) - 1) {
            filepath[i++] = *file_arg++;
        }
        filepath[i] = '\0';
    ...
    if (!f) {
        fprintf(
            stderr,
            "[Player] failed to open wav file, file not found at '%s'; or file "
            "is not mono.\n",
            filepath);
        if (f)
            sf_close(f);
    ...
    if (args && strstr(args, "loop=")) {
        ...
        if (!strcmp(v, "1") || !strcmp(v, "on") || !strcmp(v, "yes") ||
            !strcmp(v, "true"))
            loop_default = 1;
        else
            loop_default = 0;
