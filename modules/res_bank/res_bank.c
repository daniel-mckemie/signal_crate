...
static void rebuild_centers(ResBank *s) {
    ...
    if (lo < 20.0)
        lo = 20.0;
    if (hi > s->sample_rate * 0.45)
        hi = s->sample_rate * 0.45;
    for (int i = 0; i < N; i++) {
...
static void rebuild_weights(ResBank *s) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        // float w_tilt = (1.f - tilt) + 2.f*tilt * t;       // low↔high
        // emphasis
...
static void clamp_params(ResBank *s) {
    ...
    if (s->hi_hz > ny)
        s->hi_hz = ny;
    if (s->hi_hz < s->lo_hz + 1.0f)
        s->hi_hz = s->lo_hz + 1.0f;
    ...
    if (s->bands < 1)
        s->bands = 1;
    if (s->bands > RES_MAX_BANDS)
        s->bands = RES_MAX_BANDS;
}
...
static void res_bank_draw_ui(Module *m, int y, int x) {
    ...
    YELLOW();
    mvprintw(y + 1, x,
             "-/= mix _/+ q [/] lo {/} hi ;/\' bands ?/\" tilt ,/. odd </> drv "
             "9/0 rgn");
    mvprintw(y + 2, x,
             ":1[mix] :2[q] :3[lo] :4[hi] :5[bnd] :6[tilt] :7[odd] :8[drive] "
             ":9[regen]");
    BLACK();
}
        ...
        if (key == '\n') {
            ...
            if (sscanf(s->command_buffer, "%c %f", &type, &val) == 2) {
                if (type == '1')
                    ...
                    s->lo_hz = val, s->need_centers = 1;
                else if (type == '4')
                    s->hi_hz = val, s->need_centers = 1;
                else if (type == '5')
                    s->bands = (int)val, s->need_centers = 1;
                else if (type == '6')
                    s->tilt = val;
                else if (type == '7')
                    s->odd = val;
...
static void res_bank_set_osc_param(Module *m, const char *param, float value) {
    ...
    if (strcmp(param, "mix") == 0)
        s->mix = fminf(fmaxf(value, 0.0f), 1.0f);
    else if (strcmp(param, "q") == 0) {
        ...
        s->tilt = fminf(fmaxf(value, -1.0f), 1.0f);
    else if (strcmp(param, "odd") == 0)
        s->odd = fminf(fmaxf(value, -1.0f), 1.0f);
    else if (strcmp(param, "drive") == 0)
        s->drive = fminf(fmaxf(value, 0.0f), 1.0f);
    else if (strcmp(param, "regen") == 0)
        s->regen = fminf(fmaxf(value, 0.0f), 1.0f);
    else if (strcmp(param, "bands") == 0) {
        float norm = fminf(fmaxf(value, 0.0f), 1.0f);
        ...
        float min_hz = 20.0f, max_hz = 20000.0f;
        float norm = fminf(fmaxf(value, 0.0f), 1.0f);
        s->lo_hz = min_hz * powf(max_hz / min_hz, norm);
        s->need_centers = 1;
    } else if (strcmp(param, "hi") == 0) {
        float min_hz = 20.0f, max_hz = 20000.0f;
        float norm = fminf(fmaxf(value, 0.0f), 1.0f);
        s->hi_hz = min_hz * powf(max_hz / min_hz, norm);
        s->need_centers = 1;
    } else {
...
Module *create_module(const char *args, float sample_rate) {
    // defaults
    float mix = 0.5f, q = 12.f, lo = 120.f, hi = 6000.f, tilt = 0.f, odd = 0.f,
          drive = 0.2f, regen = 0.1f;
    int bands = 12;

    if (args && strstr(args, "mix="))
        sscanf(strstr(args, "mix="), "mix=%f", &mix);
    if (args && strstr(args, "q="))
        sscanf(strstr(args, "q="), "q=%f", &q);
    if (args && strstr(args, "lo="))
        sscanf(strstr(args, "lo="), "lo=%f", &lo);
    if (args && strstr(args, "hi="))
        sscanf(strstr(args, "hi="), "hi=%f", &hi);
    if (args && strstr(args, "tilt="))
        sscanf(strstr(args, "tilt="), "tilt=%f", &tilt);
    if (args && strstr(args, "odd="))
        sscanf(strstr(args, "odd="), "odd=%f", &odd);
    if (args && strstr(args, "drive="))
        sscanf(strstr(args, "drive="), "drive=%f", &drive);
    if (args && strstr(args, "regen="))
        sscanf(strstr(args, "regen="), "regen=%f", &regen);
    if (args && strstr(args, "bands="))
        sscanf(strstr(args, "bands="), "bands=%d", &bands);
