...
static void moog_filter_handle_input(Module *m, int key) {
    ...
    if (!state->entering_command) {
        ...
        case '_':
            ...
            break;
        case 'f':
            state->filt_type = (state->filt_type + 1) % 5;
            handled = 1;
            break;
        case ':':
...
static void moog_filter_set_osc_param(Module *m, const char *param,
                                      float value) {
    MoogFilter *state = (MoogFilter *)m->state;
    pthread_mutex_lock(&state->lock);
...
Module *create_module(const char *args, float sample_rate) {
    ...
    if (args && strstr(args, "type=")) {
        ...
        if (strcmp(filt_str, "LP") == 0)
            filt_type = LOWPASS;
        else if (strcmp(filt_str, "HP") == 0)
            filt_type = HIGHPASS;
        else if (strcmp(filt_str, "BP") == 0)
            filt_type = BANDPASS;
        else if (strcmp(filt_str, "notch") == 0)
            filt_type = NOTCH;
        else if (strcmp(filt_str, "res") == 0)
            filt_type = RESONANT;
        else
            fprintf(stderr, "[moog_filter] Unknown type: '%s'\n", filt_str);
    }
