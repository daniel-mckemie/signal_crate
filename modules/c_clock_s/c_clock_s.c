...
static void c_clock_draw_ui(Module *m, int y, int x) {
    ...
    char buf[64];
    memcpy(buf, s->command_buffer, sizeof(buf));
    pthread_mutex_unlock(&s->lock);

    BLUE();
    mvprintw(y, x, "[CLK:%s] ", m->name);
    CLR();
    LABEL(2, "bpm:");
    ORANGE();
    printw(" %.1f | ", bpm);
    ...
    if (!s->entering_command) {
        ...
        case ' ':
            if (has_sync) {
                ...
                handled = 1;
                // No global run change here; display_running updated in process
                // loop
            } else {
                s->running = !s->running;
                handled = 1;
...
Module *create_module(const char *args, float sample_rate) {
    ...
    if (args && strstr(args, "bpm="))
        sscanf(strstr(args, "bpm="), "bpm=%f", &bpm);
    if (args && strstr(args, "mult="))
        sscanf(strstr(args, "mult="), "mult=%f", &mult);
    if (args && strstr(args, "pw="))
        sscanf(strstr(args, "pw="), "pw=%f", &pw);
