...
static inline void pink_filter_init(PinkFilter *pf, float sample_rate) {
    const float a44[6] = {0.99886f, 0.99332f, 0.96900f,
                          0.86650f, 0.55000f, -0.7616f};
    const float g44[7] = {0.0555179f, 0.0750759f, 0.1538520f, 0.3104856f,
...
static inline float pink_filter_process(PinkFilter *pf, float white) {
    ...
    float pink = pf->b[0] + pf->b[1] + pf->b[2] + pf->b[3] + pf->b[4] +
                 pf->b[5] + pf->b[6] + white * 0.5362f;
    pf->b[6] = white * pf->g[6];
    return pink;
}
