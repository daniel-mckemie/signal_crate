...
static void c_cv_proc_draw_ui(Module *m, int y, int x) {
    ...
    LABEL(2, "k:");
    ORANGE();
    printw(" %.2f | ", s->display_k);
    CLR();

    LABEL(2, "m:");
    ORANGE();
    printw(" %.2f | ", s->display_m_amt);
    CLR();

    LABEL(2, "offset:");
    ORANGE();
    printw(" %.2f", s->display_offset);
    CLR();

    YELLOW();
    mvprintw(y + 1, x, "out: %.3f | va: %.3f | vb: %.3f | vc: %.3f", val,
             s->display_va, s->display_vb, s->display_vc);
