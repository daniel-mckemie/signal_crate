...
static void spec_hold_process(Module *m, float *in, unsigned long frames) {
    ...
    for (unsigned long i = 0; i < frames; i++) {
        ...
        if (state->hop_write_index >= HOP_SIZE) {
            ...
            for (int k = 0; k < FFT_SIZE; k++) {
                state->time_buffer[k] = state->input_buffer[idx] * hann[k];
                idx++;
                if (idx >= FFT_SIZE)
                    idx = 0;
            }
    ...
    memmove(state->output_buffer, state->output_buffer + frames,
            sizeof(float) * (FFT_SIZE - frames));
    memset(state->output_buffer + (FFT_SIZE - frames), 0,
           sizeof(float) * frames);
...
static void spec_hold_draw_ui(Module *m, int y, int x) {
    ...
    LABEL(2, "pivot:");
    ORANGE();
    printw(" %.2f Hz | ", pivot_hz);
    CLR();

    LABEL(2, "freeze:");
    ORANGE();
    printw(" %s | ", freeze ? "ON" : "OFF");
    CLR();
...
static void spec_hold_set_osc_param(Module *m, const char *param, float value) {
    ...
    if (strcmp(param, "tilt") == 0) {
        state->tilt = fminf(fmaxf(value * 2.0f - 1.0f, -1.0f),
                            1.0f); // map [0,1] → [-1,1]
...
Module *create_module(const char *args, float sample_rate) {
    ...
    state->fft_plan = fftwf_plan_dft_r2c_1d(FFT_SIZE, state->time_buffer,
                                            state->freq_buffer, FFTW_ESTIMATE);
    state->ifft_plan = fftwf_plan_dft_c2r_1d(FFT_SIZE, state->freq_buffer,
                                             state->time_buffer, FFTW_ESTIMATE);
