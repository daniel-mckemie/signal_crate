...
static void clamp_params(VCO *state) {
    ...
    case RANGE_FULL:
        ...
        break;
    case RANGE_SUPER:
        max_freq = state->sample_rate * 0.45;
        break;
    }
...
static void vco_draw_ui(Module *m, int y, int x) {
    ...
    LABEL(2, "wave:");
    ORANGE();
    printw(" %s | ", wave_names[waveform]);
    CLR();

    /* Range */
    LABEL(2, "range:");
    ORANGE();
    printw(" %s", range_names[range]);
    CLR();

    /* Secondary UI (yellow) */
    YELLOW();
    mvprintw(y + 1, x,
             "Keys: -/= (freq), _/+ (fine), [/] (amp), w (wave), r (range)");
...
static void vco_handle_input(Module *m, int key) {
    ...
    if (!state->entering_command) {
        ...
        case 'w':
            ...
            break;
        case 'r':
            state->range_mode = (state->range_mode + 1) % 5;
            handled = 1;
            break;
        case ':':
...
static void vco_set_osc_param(Module *m, const char *param, float value) {
    ...
    if (strcmp(param, "freq") == 0) {
        ...
        case RANGE_FULL:
            ...
            break;
        case RANGE_SUPER:
            max_hz = state->sample_rate * 0.45f;
            break;
        default:
            max_hz = 20000.0f;
            break;
...
Module *create_module(const char *args, float sample_rate) {
    ...
    if (args && strstr(args, "wave=")) {
        ...
        if (strcmp(wave_str, "sine") == 0)
            ...
            fprintf(stderr, "[vco] Unknown wave type: '%s'\n", wave_str);
    }
