...
static void noise_process(Module *m, float *in, unsigned long frames) {
    ...
    for (unsigned long i = 0; i < frames; i++) {
        ...
        case WHITE_NOISE:
            ...
            break;
        case PINK_NOISE:
            value = pink_filter_process(&state->pink, white);
            break;
        case BROWN_NOISE:
            value = brown_noise_process(&state->brown, white);
            break;
        }
        out[i] = amp * value;
    }
...
static void noise_draw_ui(Module *m, int y, int x) {
    ...
    LABEL(2, "type:");
    ORANGE();
    printw(" %s", noise_names[noise_type]);
    CLR();
...
static void noise_handle_input(Module *m, int key) {
    ...
    if (!state->entering_command) {
        ...
        case '-':
            ...
            break;
        case 'n':
            state->noise_type = (state->noise_type + 1) % 3;
            handled = 1;
            break;
        case ':':
...
Module *create_module(const char *args, float sample_rate) {
    ...
    if (args && strstr(args, "type=")) {
        ...
        if (strcmp(noise_str, "white") == 0)
            noise_type = WHITE_NOISE;
        else if (strcmp(noise_str, "pink") == 0)
            noise_type = PINK_NOISE;
        else if (strcmp(noise_str, "brown") == 0)
            noise_type = BROWN_NOISE;
        else
            fprintf(stderr, "[noise] Unknown type: '%s'\n", noise_str);
    }
