...
int main(int argc, char **argv) {
    ...
    const PaDeviceInfo *inputInfo =
        (inputDevice != paNoDevice) ? Pa_GetDeviceInfo(inputDevice) : NULL;
    ...
    if (argc > 1) {
        ...
        if (!f) {
            fprintf(stderr,
                    "Usage: signalcrate <patch.txt> [midi-device]\n"
    ...
    if (!has_audio) {
        fprintf(stderr,
                "[error] No modules with audio output found. Exiting.\n");
        Pa_Terminate();
        return 1;
    }
    ...
    PaStreamParameters inputParams = {
        ...
        .suggestedLatency =
            inputInfo ? inputInfo->defaultLowInputLatency : 0.01,
    ...
    PaError err = Pa_OpenStream(
        &stream, (inputDevice != paNoDevice) ? &inputParams : NULL,
        &outputParams, sample_rate, REQUESTED_FRAMES_PER_BUFFER, paClipOff,
        audio_callback, NULL);
